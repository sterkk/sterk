...
"""
import secrets, hashlib, random, json, os, re, threading, time, smtplib
...
SMTP_USER = os.environ.get('SMTP_USER', '')  # ör: sterk@gmail.com
SMTP_PASS = os.environ.get('SMTP_PASS', '')  # Gmail App Password
SMTP_FROM = os.environ.get('SMTP_FROM', '') or SMTP_USER
HAVE_SMTP = bool(SMTP_USER and SMTP_PASS)
...
def send_email(to_email, subject, html_body):
    """SMTP ile e-posta gönder. HAVE_SMTP false ise sessizce geç."""
    if not HAVE_SMTP:
        app.logger.warning(f'SMTP ayarlanmamış — e-posta gönderilemedi: {to_email}')
        return False
    try:
        ...
        app.logger.info(f'E-posta gönderildi: {to_email}')
        return True
    except Exception as e:
        app.logger.error(f'E-posta gönderim hatası: {e}')
        return False
...
def send_verification_email(email, token):
    ...
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:500px;margin:0 auto;background:#0f0f1e;color:#e2e8f0;border-radius:16px;overflow:hidden;border:1px solid #1e293b">
      ...
      <div style="padding:28px 24px">
        ...
        <p style="color:#94a3b8;font-size:14px;line-height:1.6">Merhaba! Sterk hesabınızı doğrulamak için aşağıdaki butona tıklayın:</p>
        <div style="text-align:center;margin:24px 0">
          <a href="{link}" style="display:inline-block;background:linear-gradient(135deg,#7c3aed,#6366f1);color:white;padding:12px 32px;border-radius:10px;text-decoration:none;font-weight:700;font-size:15px">✓ E-postamı Doğrula</a>
        </div>
        <p style="color:#64748b;font-size:12px">Buton çalışmazsa bu linki tarayıcınıza yapıştırın:<br>
          <a href="{link}" style="color:#7c3aed;word-break:break-all">{link}</a>
        </p>
      </div>
      <div style="background:#1a1a2e;padding:14px 24px;text-align:center;font-size:11px;color:#475569">© 2026 Sterk — Bu e-postayı siz istediniz.</div>
    </div>
    """
...
def save_file(file, subdir):
    ...
    # Cloudinary varsa buluta yükle
    if HAVE_CLOUD:
        try:
            resource_type = 'video' if ext in ('mp4','mov','webm') else 'image'
            if ext == 'pdf': resource_type = 'raw'
            result = cloudinary.uploader.upload(
                file, folder=f"sterk/{subdir}",
                public_id=fn.rsplit('.',1)[0],
                resource_type=resource_type
            )
            return result['secure_url'], ext
        except Exception as e:
            app.logger.error(f"Cloudinary upload error: {e}")
    # Fallback: yerel diske kaydet
...
def media_url(path):
    """Cloudinary URL ise direkt dön, değilse /uploads/ prefix ekle"""
    if not path: return ''
...
def register():
    if request.method=='POST':
        ...
        try:
            ...
            threading.Thread(target=send_verification_email, args=(em, tok), daemon=True).start()
...
def upload_avatar():
    ...
    try:
        ...
        c.commit(); return jsonify({'success':True,'url':media_url(path)})
    finally: release_db(c)
...
def resend_verification():
    ...
    try:
        ...
        threading.Thread(target=send_verification_email, args=(user['email'], tok), daemon=True).start()
        return jsonify({'success':True,'message':'Doğrulama bağlantısı e-posta adresinize gönderildi'})
    finally: release_db(c)
